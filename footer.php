<footer class="footer">
  <div class="footer__inner">
    ...
    <div class="theme-switchers" role="group" aria-label="Color theme">
      <button class="theme-switcher" data-theme-name="light" title="Set light theme" aria-label="Light theme">
      ...
      <button class="theme-switcher" data-theme-name="dark" title="Set dark theme" aria-label="Dark theme">
          <svg aria-hidden="true" role="presentation" width="24" height="24" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      <button class="theme-switcher" data-theme-name="auto" title="Follow system theme" aria-label="System theme">
    ...
    <div class="footer__content footer__content_align_right">
      ...
      <span class="footer__text">&copy; 2011-2026</span>
    </div>
  </div>
</footer>
...
(() => {
    const switchAriaCurrent = (themeName) => {
        ...
        themeSwitchers.forEach((themeSwitcher) => {
            const active = themeSwitcher.dataset.themeName === themeName;
            if (active) themeSwitcher.setAttribute('aria-current', 'true');
            else themeSwitcher.removeAttribute('aria-current');
            themeSwitcher.setAttribute('aria-pressed', active ? 'true' : 'false');
        });
    };

    const stored = localStorage.getItem('theme-name') || 'auto';
    document.documentElement.setAttribute('data-theme-name', stored);
    switchAriaCurrent(stored);
